...
# Common Builder image used in khulnasoft/khulnasoft
# We need gcc for cgo cross-compilation at least, we can swap to something smaller later on
ARG PROXYLIB_BUILDER=quay.io/khulnasoft/khulnasoft-builder:832f86bb0f7c7129c1536d5620174deeec645117@sha256:6dbac9f9eba3e20f8edad4676689aa8c11b172035fe5e25b533552f42dea4e9a
...
RUN --mount=type=bind,readwrite,target=/go/src/github.com/khulnasoft/proxy --mount=mode=0777,target=/khulnasoft/proxy/.cache,type=cache --mount=mode=0777,target=/go/pkg,type=cache \
    GOARCH=${TARGETARCH} make -C proxylib all && mv proxylib/libkhulnasoft.so /tmp/libkhulnasoft.so
