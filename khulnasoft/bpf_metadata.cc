...
#include "khulnasoft/api/bpf_metadata.pb.validate.h"
...
public:
  ...
  Network::ListenerFilterFactoryCb createListenerFilterFactoryFromProto(
    ...
    options->push_back(std::make_shared<Khulnasoft::SocketMarkOption>(mark, 0));
...
createHostMap(Server::Configuration::ListenerFactoryContext& context) {
  return context.singletonManager().getTyped<const Khulnasoft::PolicyHostMap>(
      SINGLETON_MANAGER_REGISTERED_NAME(khulnasoft_host_map), [&context] {
        auto map = std::make_shared<Khulnasoft::PolicyHostMap>(context);
...
createPolicyMap(Server::Configuration::FactoryContext& context, Khulnasoft::CtMapSharedPtr& ct) {
  return context.singletonManager().getTyped<const Khulnasoft::NetworkPolicyMap>(
...
Config::Config(const ::khulnasoft::BpfMetadata& config,
  ...
  if (bpf_root.length() > 0) {
    ct_maps_ = context.singletonManager().getTyped<Khulnasoft::CtMap>(
    ...
    ipcache_ = context.singletonManager().getTyped<Khulnasoft::IPCache>(
...
bool Config::getMetadata(Network::ConnectionSocket& socket) {
  ...
  if (is_l7lb_) {
    if (use_original_source_address_) {
      ...
      if (!ip) {
        ...
        ENVOY_LOG(warn,
                  "khulnasoft.bpf_metadata (north/south L7 LB): No local IP source address configured "
                  "for the family of {}",
                  pod_ip);
        return false;
      }
      ...
      if (new_id == Khulnasoft::ID::WORLD) {
        // No security ID available for the configured source IP
        ENVOY_LOG(warn,
                  "khulnasoft.bpf_metadata (north/south L7 LB): Unknown local IP source address "
                  "configured: {}",
                  pod_ip);
        return false;
      }
    ...
    // same node.
               ...
               !(destination_identity & Khulnasoft::ID::LocalIdentityFlag) &&
               destination_identity != Khulnasoft::ID::WORLD && !npmap_->exists(other_ip))) {
  ...
  if (policy == nullptr) {
    ENVOY_LOG(warn, "khulnasoft.bpf_metadata ({}): No policy found for {}",
  ...
  if (policy->useProxylib(is_ingress_, dip->port(),
    ...
    ENVOY_LOG(info, "khulnasoft.bpf_metadata: setRequestedApplicationProtocols(..., {})", l7proto);
  }
  ...
  socket.addOption(std::make_shared<Khulnasoft::SocketOption>(
