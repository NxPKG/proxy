...
bool IPCache::Open() {
  ...
  std::string path(bpf_root_ + "/tc/globals/khulnasoft_ipcache");
  if (!open(path)) {
    ENVOY_LOG(info, "khulnasoft.ipcache: Cannot open ipcache map at {}", path);
    return false;
  }
  ENVOY_LOG(debug, "khulnasoft.ipcache: Opened ipcache.");
  return true;
}
