...
protected:
  ...
  public:
    HttpNetworkPolicyRule(const NetworkPolicyMap& parent,
                          const khulnasoft::HttpNetworkPolicyRule& rule) {
  ...
  public:
    L7NetworkPolicyRule(const khulnasoft::L7NetworkPolicyRule& rule) : name_(rule.name()) {
  ...
  public:
    PortNetworkPolicyRule(const NetworkPolicyMap& parent, const khulnasoft::PortNetworkPolicyRule& rule)
      ...
      for (const auto& remote : rule.remote_policies()) {
        ENVOY_LOG(trace, "Khulnasoft L7 PortNetworkPolicyRule(): Allowing remote {} by rule: {}",
                  remote, name_);
      ...
      for (const auto& sni : rule.server_names()) {
        ENVOY_LOG(trace, "Khulnasoft L7 PortNetworkPolicyRule(): Allowing SNI {} by rule {}", sni,
                  name_);
    ...
    bool useProxylib(std::string& l7_proto) const override {
      if (l7_proto_.length() > 0) {
        ENVOY_LOG(trace, "Khulnasoft L7 PortNetworkPolicyRules::useProxylib(): returning {}",
                  l7_proto_);
        l7_proto = l7_proto_;
        return true;
      }
...
NetworkPolicyMap::NetworkPolicyMap(Server::Configuration::FactoryContext& context)
    : tls_map_(context.threadLocal()),
      ...
      name_(fmt::format("khulnasoft.policymap.{}.{}.", local_ip_str_, ++instance_id_)),
...
void NetworkPolicyMap::startSubscription(Server::Configuration::FactoryContext& context) {
  subscription_ = subscribe("type.googleapis.com/khulnasoft.NetworkPolicy", context.localInfo(),
...
void ThreadLocalPolicyMap::Update(std::vector<std::shared_ptr<PolicyInstanceImpl>>& added,
  ...
  ENVOY_LOG(trace,
            "Khulnasoft L7 NetworkPolicyMap::onConfigUpdate(): Starting "
  ...
  for (const auto& policy_name : deleted) {
    ENVOY_LOG(trace, "Khulnasoft deleting removed network policy for endpoint {}", policy_name);
  ...
  if (to_be_added->size() == 0 && to_be_deleted->size() == 0 && cts_to_be_closed->size() == 0) {
    ...
    tls_map_.runOnAllThreads(
        [to_be_added, to_be_deleted, version_info](OptRef<ThreadLocalPolicyMap> npmap) {
          ...
          if (!npmap.has_value()) {
            ENVOY_LOG(debug,
                      "Khulnasoft L7 NetworkPolicyMap::onConfigUpdate(): npmap has no value "
                      "for version {}",
                      version_info);
            return;
          }
