...
public:
  ...
  void setTransportSocketCallbacks(Network::TransportSocketCallbacks& callbacks) override {
    // Get the Khulnasoft socket option from the callbacks in order to get the TLS
    // configuration
    const auto option = Khulnasoft::GetSocketOption(callbacks.connection().socketOptions());
    if (option) {
      ...
      if (!option->ingress_) {
        ...
        if (dst_address) {
          ...
          if (dip) {
            ...
            ENVOY_LOG_MISC(warn, "khulnasoft.tls_wrapper: Non-IP destination address: {}",
                           dst_address->asString());
          }
        } else {
          ENVOY_LOG_MISC(warn, "khulnasoft.tls_wrapper: No destination address");
        }
      }
      ...
      if (port_policy != nullptr) {
        ...
        if (ctx) {
          ...
          ENVOY_LOG_MISC(debug,
                         "khulnasoft.tls_wrapper: Could not get {} TLS context for port {}, defaulting "
                         "to raw socket",
                         state_ == Extensions::TransportSockets::Tls::InitialState::Client
                             ? "client"
                             : "server",
        ...
        ENVOY_LOG_MISC(warn, "khulnasoft.tls_wrapper: Policy not found for port {}!", destination_port);
      }
    } else if (!option) {
      ENVOY_LOG_MISC(warn, "khulnasoft.tls_wrapper: Khulnasoft socket option not found!");
    }
  }
