...
#include "khulnasoft/api/network_filter.pb.validate.h"
...
Config::Config(const ::khulnasoft::NetworkFilter& config,
  ...
  if (access_log_path.length()) {
    access_log_ = Khulnasoft::AccessLog::Open(access_log_path);
  }
...
Network::FilterStatus Instance::onNewConnection() {
  ENVOY_LOG(debug, "khulnasoft.network: onNewConnection");
  ...
  if (!option) {
    ENVOY_CONN_LOG(warn, "khulnasoft.network: Khulnasoft Socket Option not found", conn);
  ...
  callbacks_->addUpstreamCallback([this, option, sni](Upstream::HostDescriptionConstSharedPtr host,
    ...
    ENVOY_LOG(trace, "khulnasoft.network: in upstream callback");
    auto& conn = callbacks_->connection();
    ...
    if (nullptr == dst_address) {
      ENVOY_LOG(warn, "khulnasoft.network (egress): No destination address ");
      return false;
    }
    if (!option->ingress_) {
      const auto dip = dst_address->ip();
      if (!dip) {
        ENVOY_LOG_MISC(warn, "khulnasoft.network: Non-IP destination address: {}",
      ...
      if (option->ingress_source_identity_ != 0) {
        ...
        if (port_policy_ == nullptr ||
          ...
          ENVOY_CONN_LOG(debug,
                         "khulnasoft.network: ingress policy drop for source identity: {} port: {}",
    ...
    if (port_policy_ == nullptr) {
      ENVOY_CONN_LOG(warn, "khulnasoft.network: Policy NOT FOUND for id: {} port: {}", conn,
    ...
    if (!port_policy_->Matches(sni, option->ingress_ ? option->identity_ : destination_identity)) {
      ...
      ENVOY_CONN_LOG(warn, "khulnasoft.network: Policy DENY on id: {} port: {}", conn,
                     option->ingress_ ? option->identity_ : destination_identity, destination_port);
      return false;
    }
...
Network::FilterStatus Instance::onData(Buffer::Instance& data, bool end_stream) {
  ENVOY_LOG(trace, "khulnasoft.network: onData {} bytes, end_stream: {}", data.length(), end_stream);
  ...
  if (go_parser_) {
    ...
    ENVOY_CONN_LOG(trace, "khulnasoft.network::onData: \'GoFilter::OnIO\' returned {}", conn, res);
