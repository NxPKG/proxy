...
public:
  ...
  bool setOption(Network::Socket& socket,
    ...
    auto status = khulnasoft_calls.setsockopt(socket.ioHandle().fdDoNotUse(), SOL_SOCKET, SO_MARK,
                                          &mark_, sizeof(mark_));
    ...
    if (source_address || identity_ == 0) {
      ...
      if (*ipVersion == Network::Address::IpVersion::v4) {
        auto status = khulnasoft_calls.setsockopt(socket.ioHandle().fdDoNotUse(), SOL_IP,
        ...
        auto status = khulnasoft_calls.setsockopt(socket.ioHandle().fdDoNotUse(), SOL_IPV6,
...
public:
  SocketOption(PolicyInstanceConstSharedPtr policy, uint32_t mark, uint32_t ingress_source_identity,
    ...
    ENVOY_LOG(debug,
              "Khulnasoft SocketOption(): source_identity: {}, "
...
static inline const Khulnasoft::SocketOption*
GetSocketOption(const Network::Socket::OptionsSharedPtr& options) {
  if (options) {
    for (const auto& option : *options) {
      auto opt = dynamic_cast<const Khulnasoft::SocketOption*>(option.get());
      if (opt) {
        return opt;
      }
    }
