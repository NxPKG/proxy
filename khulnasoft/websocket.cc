...
#include "khulnasoft/api/websocket.pb.validate.h"
...
Network::FilterStatus Instance::onNewConnection() {
  ENVOY_LOG(debug, "khulnasoft.network.websocket: onNewConnection");
...
Network::FilterStatus Instance::onData(Buffer::Instance& data, bool end_stream) {
  ENVOY_LOG(debug, "khulnasoft.network.websocket: onData {} bytes, end_stream: {}", data.length(),
            end_stream);
  if (codec_) {
    if (config_->client_) {
...
Network::FilterStatus Instance::onWrite(Buffer::Instance& data, bool end_stream) {
  ENVOY_LOG(trace, "khulnasoft.network.websocket: onWrite {} bytes, end_stream: {}", data.length(),
            end_stream);
  if (codec_) {
    if (config_->client_) {
      codec_->decode(data, end_stream);
