...
#include "khulnasoft/api/l7policy.pb.validate.h"
...
public:
  ...
  createFilterFactoryFromProto(const Protobuf::Message& proto_config, const std::string&,
    ...
    auto config = std::make_shared<Khulnasoft::Config>(
        MessageUtil::downcastAndValidate<const ::khulnasoft::L7Policy&>(
  ...
  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<::khulnasoft::L7Policy>();
  }

  std::string name() const override { return "khulnasoft.l7policy"; }
};
...
Config::Config(const std::string& access_log_path, const std::string& denied_403_body,
    ...
    : time_source_(context.timeSource()), stats_{ALL_KHULNASOFT_STATS(
                                              POOL_COUNTER_PREFIX(context.scope(), "khulnasoft"))},
...
Config::Config(const ::khulnasoft::L7Policy& config, Server::Configuration::FactoryContext& context)
...
Http::FilterHeadersStatus AccessFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  if (!conn) {
    ENVOY_LOG(warn, "khulnasoft.l7policy: No connection");
  ...
  const auto option = Khulnasoft::GetSocketOption(socketOptions);
  if (!option) {
    ENVOY_LOG(warn, "khulnasoft.l7policy: Khulnasoft Socket Option not found");
    // Return a 500 response
    callbacks_->sendLocalReply(Http::Code::InternalServerError, "", nullptr, absl::nullopt,
                               absl::string_view());
    return Http::FilterHeadersStatus::StopIteration;
  ...
  callbacks_->addUpstreamCallback([this, option](Http::RequestHeaderMap& headers,
    ...
    if (nullptr == dst_address) {
      ENVOY_LOG(warn, "khulnasoft.l7policy: No destination address");
      return false;
    }
    const auto dip = dst_address->ip();
    if (!dip) {
      ENVOY_LOG_MISC(warn, "khulnasoft.l7policy: Non-IP destination address: {}",
    ...
    if (!policy) {
      ENVOY_LOG(debug, "khulnasoft.l7policy: No policy found for pod {}, defaulting to DENY",
                option->pod_ip_);
      return false;
    }
    ...
    if (option->ingress_source_identity_ != 0) {
      allowed_ = policy->Allowed(true, option->port_, option->ingress_source_identity_, headers,
                                 log_entry_);
      ENVOY_LOG(debug,
                "khulnasoft.l7policy: Ingress from {} policy lookup for endpoint {} for port {}: {}",
    ...
    if (allowed_) {
      allowed_ = policy->Allowed(option->ingress_, destination_port,
                                 ...
                                 headers, log_entry_);
      ENVOY_LOG(debug, "khulnasoft.l7policy: {} ({}->{}) policy lookup for endpoint {} for port {}: {}",
